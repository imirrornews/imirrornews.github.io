...
$(document).ready(function() {
    /////////////////////////////////////////////////////
    // Global variable
    //var webserviceUrl = "http://localhost/live/imovie/api/v1/";
    var webserviceUrl = "https://mmsoftware100.com/live/imovie/api/v1/";
    ...
    var MovieViewEdit = Backbone.View.extend({
        ...
        episodeSelect : function(event){
            ...
            let modelData = this.model.toJSON();
            let episode = $(event.currentTarget).val();
            let seasonIndex = $(event.currentTarget).attr('data-season');
            let episodeIndex = $(event.currentTarget).attr('data-episode');
            modelData.season_list[seasonIndex].episode_list[episodeIndex].episode = episode;
            this.model.updateMovie(modelData);
            console.log(this.model);
        },
        ...
        resolutionSelect : function(event){
            ...
            console.log(modelData.season_list[seasonIndex]);
            modelData.season_list[seasonIndex].episode_list[episodeIndex].file_list[fileIndex].resolution = resolution;
            this.model.updateMovie(modelData);
            console.log(this.model);
        },
        fileName : function(event){
            ...
            let episodeIndex = $(event.currentTarget).attr('data-episode');
            let fileIndex = $(event.currentTarget).attr('data-file');
            modelData.season_list[seasonIndex].episode_list[episodeIndex].file_list[fileIndex].name = fileName;
            this.model.updateMovie(modelData);
            console.log(this.model);
        },
        fileSize : function(event){
            ...
            let episodeIndex = $(event.currentTarget).attr('data-episode');
            let fileIndex = $(event.currentTarget).attr('data-file');
            modelData.season_list[seasonIndex].episode_list[episodeIndex].file_list[fileIndex].size = fileSize;
            this.model.updateMovie(modelData);
            console.log(this.model);
        },
        addFileButton : function(event ){
            ...
            // traversing လုပ်ပြီးတော့ ဘေးက လူရဲ့ အချက်အလက်တွေကို ကြည့်မှ ရမှာ ၊ အခုက ဘယ် season , ဘယ် episode အတွက် ဖိုင်အသစ်ထည့်မှာလဲ ဆိုတာ
            // အရင်က $this element နဲ့ ယူလို့ ရတာ ရှိတယ် ပြန်လိုက်ရှာကြည့်ရမယ်။
            //console.log($(this));
            let season = $(event.currentTarget).attr('data-season');
            let episode = $(event.currentTarget).attr('data-episode');
            //let txt = this.$('.addFileButton').text();
            console.log("season " + season);
            console.log("episode " + episode);
            //console.log("txt " + txt);
            //$(event.currentTarget).hide();
            modelData.season_list[season].episode_list[episode].file_list.push(newFile);
            console.log(modelData);
            this.model.updateMovie(modelData);
        ...
        addEpisodeButton : function(event ){
            ...
            let newEpisode = makeEpisode(0,'',0,'',[newFile]);
